...
def parse_cmdargs():
    parser = argparse.ArgumentParser(
        description = 't-sne visualization code'
    )
    parser.add_argument(
        '--root',
...
def str2floatarray(df):
    ...
    return x, y
def tsne(cmdargs, i, XYZ, labels_XYZ):
    n_components = 2
    # TSNE 모델의 인스턴스를 만듭니다.
    model1 = TSNE(n_components=n_components, perplexity=i, n_iter=500)
    embedded = model1.fit_transform(XYZ)
    df_tsne = pd.DataFrame(np.concatenate((embedded, labels_XYZ.reshape(-1, 1)), axis=1), columns = ['1st component', '2nd component', 'label'])
    ...
    plt.legend()
    plt.savefig(cmdargs.root + "/t-sne{num}.PNG".format(num=i))
    plt.close()
def main():
